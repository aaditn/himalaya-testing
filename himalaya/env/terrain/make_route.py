...
def route(res=256, extent=12.0, lane_w=1.1, wall_h=0.70, wall_w=0.9,
          rough=0.10, seed=0):
  ...
  # Lane flat, then a steep rise across the wall band, then broken ground.
  t = np.clip((dist - half_lane) / max(1e-6, wall_w), 0.0, 1.0)
  h = wall_h * (t ** 0.55)                          # sharp near the lane edge
